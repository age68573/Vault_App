...
<div class="page-header d-print-none">
  <div class="container-xl">
    <div class="row g-2 align-items-center">
      <div class="col">
        <h2 class="page-title">MongoDB 資料瀏覽器</h2>
        <c:if test="${not noCred and not empty currentCred}">
          <div class="mt-1 d-flex align-items-center gap-2">
            ...
            <span class="badge bg-info-lt text-info small"
                  data-ttl="${currentCred.remainingTtlSeconds}">
              TTL <c:out value="${currentCred.remainingTtlSeconds}"/>s
            </span>
          </div>
        </c:if>
      </div>
  ...
  <div class="container-xl">
    ...
    <c:if test="${not empty dbError}">
      <div class="alert alert-danger mb-4">
        <i class="bi bi-exclamation-triangle-fill me-2"></i><c:out value="${dbError}"/>
      </div>
    </c:if>
    <c:if test="${not empty param.msg}">
      <div class="alert alert-success alert-dismissible mb-4">
        <i class="bi bi-check-circle-fill me-2"></i><c:out value="${param.msg}"/>
        ...
        <div class="col-12 col-md-3 col-lg-2">
          ...
          <div class="card">
            <div class="card-header py-2">
              <h4 class="card-title mb-0 small">
                <i class="bi bi-search me-1"></i>指定 Collection
              </h4>
            </div>
              ...
              <form method="get" action="${pageContext.request.contextPath}/data">
                <div class="input-group input-group-sm">
                  <input type="text" class="form-control"
                         name="collection" placeholder="輸入名稱..."
              ...
              <div class="card mb-3">
                <div class="card-header">
                  <h3 class="card-title">
                    ...
                    <span class="font-monospace"><c:out value="${selectedCollection}"/></span>
                  </h3>
                ...
                <div class="card-body">
                  <form method="get" action="${pageContext.request.contextPath}/data"
                    ...
                    <div class="col-md-7">
                      <label class="form-label small">篩選條件（MongoDB JSON）</label>
                    ...
                    <div class="col-md-2">
                      <label class="form-label small">筆數限制</label>
                      ...
                      <tbody>
                        <c:forEach var="doc" items="${documents}" varStatus="status">
                          <tr>
                            ...
                            <td>
                              <code class="json-block"><c:out value="${doc}"/></code>
                            </td>
                            <td class="text-center">
                              <form method="post" action="${pageContext.request.contextPath}/data"
                                    onsubmit="return confirm('確定要刪除這筆文件？')">
                  ...
                  <div class="accordion-item border-0">
                    <h2 class="accordion-header">
                      <button class="accordion-button collapsed" type="button"
                              data-bs-toggle="collapse" data-bs-target="#collapseInsert">
                    ...
                    <div id="collapseInsert" class="accordion-collapse collapse">
                      <div class="accordion-body">
                        <form method="post" action="${pageContext.request.contextPath}/data">
                          ...
                          <input type="hidden" name="collection" value="${selectedCollection}">
                          <div class="mb-3">
                            <label class="form-label">文件內容（JSON 格式）</label>
                            <textarea class="form-control font-monospace"
                                      name="documentJson" rows="5"
                                      placeholder='{"name": "範例", "value": 123}'
                                      required></textarea>
                            <div class="form-hint">無需指定 _id，MongoDB 將自動產生</div>
                          </div>
                ...
                <div class="card-body">
                  <div class="empty">
                    <div class="empty-icon">
                      <i class="bi bi-arrow-left-circle" style="font-size:2.5rem; color:var(--tblr-secondary);"></i>
                    </div>
                    <p class="empty-title">尚未選擇 Collection</p>
                    <p class="empty-subtitle text-muted">請從左側選擇或輸入 Collection 名稱以開始操作</p>
                  </div>
                </div>
              </div>
            </c:otherwise>
...
document.querySelectorAll('.delete-filter-input').forEach(input => {
  try {
    ...
    if (doc._id) input.value = JSON.stringify({ _id: doc._id });
  } catch (e) {}
});
</script>
