...
**注意：** 應用程式在 Session 中維護 `knownLeaseIds` 清單，不依賴 `LIST /v1/sys/leases`（需要 root 權限），改為對每個已知 leaseId 逐一查詢。
...
| 登出（撤銷 Token） | `POST /v1/auth/token/revoke-self` |
...
`LIST /v1/sys/leases` 需要 root token 或 sudo 權限。本應用程式改為在 Session 中自行維護 `knownLeaseIds` 清單，對每個 Lease ID 逐一呼叫 `lookup` 查詢，繞過此限制。
